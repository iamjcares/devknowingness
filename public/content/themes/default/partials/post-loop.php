    ...
    <div class="col-lg-4 col-md-6 col-sm-6 col-xs-12">
        <article class="block">
            <a class="block-thumbnail" href="<?= ($settings->enable_https) ? secure_url('post') : URL::to('post') ?><?= '/' . $post->slug ?>">
                ...
                <img src="<?= Knowingness\Libraries\ImageHandler::getImage($post->image, 'medium') ?>">
                <div class="details">
                    <h2><?= $post->title; ?></h2>
                    <span><?= Knowingness\Libraries\TimeHelper::convert_seconds_to_HMS($post->duration); ?></span>
                </div>
            </a>
            <div class="block-contents">
                <p class="date"><?= date("F jS, Y", strtotime($post->created_at)); ?></p>
                <p class="desc"><?php if (strlen($post_description) > 90) {
